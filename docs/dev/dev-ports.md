...
the second `pnpm dev:api` dies on `EADDRINUSE`, and the second `pnpm docker:up` cannot bind
5432. This page assigns each repo its own block of ports.
...
| 3 | `POSTGRES_PORT` ↔ `DATABASE_URL` (and `DIRECT_URL`) | **Silent and worst.** A stale `DATABASE_URL` connects to *another repo's* database. No error anywhere. |
