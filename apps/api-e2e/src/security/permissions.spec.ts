...
describe('CRITICAL: Permission Enforcement', () => {
  let owner: TestUser & { organizationId: string; roleId: string }
  let admin: TestUser & { organizationId: string; roleId: string }
  let member: TestUser & { organizationId: string; roleId: string }
  let roleIds: { owner: string; admin: string; member: string }
  ...
  beforeAll(async () => {
    ...
    owner = {
      ...
      email: registerResponse.data.data.register.user.emails?.find((e: any) => e.primary)?.email || ownerData.email,
    ...
    `
    const orgResponse = await TestHelpers.authenticatedGraphql(
      createOrgMutation,
      owner,
    ...
    `, owner, { organizationId: owner.organizationId })
    ...
    `
    const membershipResponse = await TestHelpers.authenticatedGraphql(
    ...
    admin = {
      ...
      email: adminRegResponse.data.data.register.user.emails?.find((e: any) => e.primary)?.email || adminData.email,
    ...
    member = {
      ...
      email: memberRegResponse.data.data.register.user.emails?.find((e: any) => e.primary)?.email || memberData.email,
    ...
    if (adminInviteResponse.data.errors) {
      ...
      throw new Error(`Failed to create admin invitation: ${JSON.stringify(adminInviteResponse.data.errors)}`)
    }
    ...
    if (memberInviteResponse.data.errors) {
      ...
      throw new Error(`Failed to create member invitation: ${JSON.stringify(memberInviteResponse.data.errors)}`)
    }
    ...
    const adminInviteDetails = await TestHelpers.graphql(adminInviteDetailsQuery, { token: adminToken })
    console.log('\n🔍 Admin invitation details:', JSON.stringify(adminInviteDetails.data.data, null, 2))
    ...
    if (adminAcceptResponse.data.errors) {
      ...
      throw new Error(`Failed to accept admin invitation: ${JSON.stringify(adminAcceptResponse.data.errors)}`)
    }
    ...
    const adminRoleCheck = await TestHelpers.authenticatedGraphql(
      adminRoleCheckQuery,
      owner,
      { organizationId: owner.organizationId }
    )

    const adminMembership = adminRoleCheck.data.data.userOrganizationMembers.find((m: any) => m.userId === admin.id)
    console.log(`\n🔍 Admin role after acceptance: ${adminMembership?.role?.name || 'NOT FOUND'}`)
    console.log(`  - Admin user ID: ${admin.id}`)
    console.log(`  - All members:`, adminRoleCheck.data.data.userOrganizationMembers.map((m: any) => ({ userId: m.userId, role: m.role.name })))
    ...
    if (memberAcceptResponse.data.errors) {
      ...
      throw new Error(`Failed to accept member invitation: ${JSON.stringify(memberAcceptResponse.data.errors)}`)
    }
  ...
  describe('Owner Permissions', () => {
    ...
    it('owner can delete organization', async () => {
      ...
      const tempOwner = {
        ...
        email: regResponse.data.data.register.user.emails?.find((e: any) => e.primary)?.email || tempOwnerData.email,
      ...
      `, tempOwner as TestUser, { input: { name: 'Temp Org To Delete' } })
  ...
  describe('Admin Permissions', () => {
    ...
    it('admin can invite members', async () => {
      ...
      const memberCheck = await TestHelpers.authenticatedGraphql(
        membersQuery,
        admin,
        { organizationId: owner.organizationId }
      )
    ...
    it('admin cannot change member roles', async () => {
      ...
      const membersResponse = await TestHelpers.authenticatedGraphql(
        membersQuery,
        admin,
        { organizationId: owner.organizationId }
      )
