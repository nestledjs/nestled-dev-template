...
describe('CRITICAL: Multi-Tenant Data Isolation', () => {
  let org1User: TestUser & { organizationId: string }
  let org2User: TestUser & { organizationId: string }
  ...
  beforeAll(async () => {
    ...
    const user1Email = response1.data.data.register.user.emails?.find((e: any) => e.primary)?.email || user1Data.email
    const user2Email = response2.data.data.register.user.emails?.find((e: any) => e.primary)?.email || user2Data.email
    ...
    const org1Response = await TestHelpers.authenticatedGraphql(
      createOrgMutation,
      org1User,
      { input: { name: 'Organization One' } }
    )
    const org2Response = await TestHelpers.authenticatedGraphql(
      createOrgMutation,
      org2User,
    ...
    const org1MembersResponse = await TestHelpers.authenticatedGraphql(
      membersQuery,
      org1User,
      { organizationId: org1User.organizationId }
    )
    const org2MembersResponse = await TestHelpers.authenticatedGraphql(
      membersQuery,
      org2User,
  ...
  describe('Query Isolation', () => {
    ...
    it('should not allow querying another organization\'s members', async () => {
      ...
      if (response.data.errors) {
        ...
        expect(response.data.errors[0].message).toMatch(/forbidden|unauthorized|not found|not a member/i)
      } else {
  ...
  describe('Update Isolation', () => {
    it('should not allow updating another organization\'s data', async () => {
      ...
      // Should return error
      expect(response.data.errors).toBeDefined()
      expect(response.data.errors[0].message).toMatch(/forbidden|unauthorized|not found|permission/i)
    })
  ...
  describe('Delete Isolation', () => {
    ...
    it('should not allow deleting another organization', async () => {
      ...
      // Should return error
      expect(response.data.errors).toBeDefined()
      expect(response.data.errors[0].message).toMatch(/forbidden|unauthorized|not found|permission|owner/i)
    })
  })
  ...
  describe('Organization Switching', () => {
    it('should update context when switching organizations', async () => {
      ...
      const multiOrgUser = {
        ...
        email: registerResponse.data.data.register.user.emails?.find((e: any) => e.primary)?.email || invitedUserData.email,
      ...
      `, org1User, { organizationId: org1User.organizationId })

      const memberRole = rolesResponse.data.data.organizationRoles.find((r: any) => r.name === 'Member')
  ...
  describe('Direct ID Manipulation Protection', () => {
    it('should ignore manual organizationId in create operations', async () => {
      ...
      const user2PrefsResponse = await TestHelpers.authenticatedGraphql(
        prefsQuery,
        org2User
      )
