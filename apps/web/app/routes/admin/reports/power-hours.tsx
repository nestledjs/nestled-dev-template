...
import { type AdminTopChaptersByPowerHoursQueryVariables, useAdminTopChaptersByPowerHoursQuery } from '@nestled-template/shared/sdk'

// Interface for power hour stats data
interface ChapterPowerHourData {
  chapterId: string
  chapterName: string
  memberCount: number
...
export default function AdminPowerHoursReport() {
  ...
  const filters: AdminTopChaptersByPowerHoursQueryVariables['input'] = {
  ...
  // Call the query hook for Power Hours
  const { data, loading, error } = useAdminTopChaptersByPowerHoursQuery({
  ...
  const chapters = data?.adminTopChaptersByPowerHours || []
  const chapterList = Array.isArray(chapters) ? (chapters as ChapterPowerHourData[]) : []
      ...
      <div className="px-4 sm:px-6 lg:px-8">
        <div className="sm:flex sm:items-center">
          <div className="sm:flex-auto">
            ...
            <h1 className="text-2xl font-semibold text-gray-900">Top 5 Chapters by Power Hours</h1>
            <p className="mt-2 text-sm text-gray-700">
              A list of the top 5 chapters ranked by total accepted Power Hours, filterable by date range and chapter
              size.
            </p>
          </div>
                ...
                <table className="min-w-full divide-y divide-gray-300">
                  <thead className="bg-gray-50">
                    <tr>
                      ...
                      <th scope="col" className="px-3 py-3.5 text-left text-sm font-semibold text-gray-900">
                        Total Accepted Power Hours
                      </th>
                      {/* Update table heading */}
                      <th scope="col" className="px-3 py-3.5 text-left text-sm font-semibold text-gray-900">
                        Average Power Hours per Member
                      </th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-gray-200 bg-white">
                    ...
                    {chapterList.map(
                      (
                        chapter,
                        index, // chapter type is ChapterPowerHourData
                      ) => (
                        <tr key={chapter.chapterId}>
                          ...
                          {/* Display totalPowerHours */}
                          <td className="whitespace-nowrap px-3 py-4 text-sm text-gray-500">
                            {Number(chapter.totalPowerHours || 0).toLocaleString()}
                          </td>
                          {/* Calculate and display average power hours */}
                          <td className="whitespace-nowrap px-3 py-4 text-sm text-gray-500">
                            {chapter.avgPowerHoursPerMember}
                          </td>
                        </tr>
                      ),
                    )}
