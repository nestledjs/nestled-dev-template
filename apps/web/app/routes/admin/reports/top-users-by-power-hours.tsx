...
import { type AdminTopUsersByPowerHoursQueryVariables, useAdminTopUsersByPowerHoursQuery } from '@nestled-template/shared/sdk'

// Interface for user power hour stats data
interface UserPowerHourData {
  id: string
  firstName: string
  lastName: string
  chapterName?: string
...
export default function AdminTopUsersByPowerHoursReport() {
  ...
  const filters: AdminTopUsersByPowerHoursQueryVariables['input'] = {
  ...
  // Call the query hook for Top Users by Power Hours
  const { data, loading, error } = useAdminTopUsersByPowerHoursQuery({
  ...
  const users = data?.adminTopUsersByPowerHours || []
  const userList = Array.isArray(users) ? (users as UserPowerHourData[]) : []
      ...
      <div className="px-4 sm:px-6 lg:px-8">
        <div className="sm:flex sm:items-center">
          <div className="sm:flex-auto">
            <h1 className="text-2xl font-semibold text-gray-900">Top Users by Power Hours</h1>
            <p className="mt-2 text-sm text-gray-700">
              A list of the top users ranked by total Power Hours attended, filterable by date range and chapter size.
            </p>
          </div>
                  ...
                  <tbody className="divide-y divide-gray-200 bg-white">
                    {userList.map((user, index) => (
                      <tr key={user.id}>
                        ...
                        <td className="whitespace-nowrap px-3 py-4 text-sm text-gray-500">
                          {Number(user.powerHourCount || 0).toLocaleString()}
                        </td>
                      </tr>
                    ))}
