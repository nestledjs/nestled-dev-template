...
Session state is controlled by paired env vars that live on *different services*:
...
clear creates a *second* host-scoped cookie and leaves the real one behind — the "two-cookie bug"
...
**Pattern:** schema-annotation security fixes belong in the *generator package*
...
- **Sanitize-order bug:** `sanitizeFieldValue` converts objects to `''`; arrays are objects, so
  enum-array values were wiped *after* correct detection. Fixed by moving enum-array handling
...
- **Field-type inference:** substring match on field names made `validateEmailToken` render as an
  ...
  `14ba93d`). Fixed to match the *last camelCase/snake word* only.
...
- `c211c55` — `list_organizations` on *personal* (unscoped) tokens returns the user's own orgs;
  org-scoped tokens never register that tool. Tool registration differs by token type.
- `dec6b7a` — RBAC guard on shared MCP UI + *seed permissions* fix: MCP features fail silently if
